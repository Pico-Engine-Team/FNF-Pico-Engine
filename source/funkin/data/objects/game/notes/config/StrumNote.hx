...
class StrumNote extends FlxSprite {
	...
	public function reloadNote()
	{
		...
		useRGBShader = (noteSkinConfig == null || noteSkinConfig.allowRGB) && (PlayState.SONG == null || !PlayState.SONG.disableNoteRGB);
		rgbShader.enabled = false;
		...
		if(usePixelSkin)
		{
			...
			if(Note.noteSkinAtlasExists(assetPath, noteSkinConfig != null ? noteSkinConfig.directory : null))
			{
				frames = Note.loadNoteSkinAtlas(assetPath, noteSkinConfig != null ? noteSkinConfig.directory : null);
			}
			else
			{
				if(!Note.noteSkinImageExists(assetPath, noteSkinConfig != null ? noteSkinConfig.directory : null))
					assetPath = 'noteSkins/pixel/NOTE_assets';
				var graphic = Note.loadNoteSkinGraphic(assetPath, noteSkinConfig != null ? noteSkinConfig.directory : null);
				if(graphic == null)
					return;
		...
		else
		{
			...
			frames = Note.noteSkinAtlasExists(assetPath, noteSkinConfig != null ? noteSkinConfig.directory : null) ? Note.loadNoteSkinAtlas(assetPath, noteSkinConfig != null ? noteSkinConfig.directory : null) : null;
			if(frames == null)
				return;
